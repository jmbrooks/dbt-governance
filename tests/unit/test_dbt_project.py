...
def test_manifest_loading(dbt_project: DbtProject, mock_manifest_data: dict, tmp_path: Path) -> None:
    ...
    with patch("dbt_governance.dbt_project.Manifest.from_dict") as mock_from_dict:
        ...
        loaded_manifest = dbt_project.load_manifest()
        mock_from_dict.assert_called_once_with(mock_manifest_data)
        assert loaded_manifest == mock_manifest


def test_manifest_file_not_found(dbt_project: DbtProject) -> None:
    """Test behavior when the manifest file is not found."""
    with pytest.raises(FileNotFoundError, match="Manifest file not found"):
        dbt_project.load_manifest()


def test_dbt_version(dbt_project: DbtProject, mock_manifest_data: dict, tmp_path: Path) -> None:
    ...
    with patch("dbt_governance.dbt_project.Manifest.from_dict") as mock_from_dict:
        ...
        assert dbt_project.dbt_version == "1.8.0"


def test_generated_at(dbt_project: DbtProject, mock_manifest_data: dict, tmp_path: Path) -> None:
    ...
    with patch("dbt_governance.dbt_project.Manifest.from_dict") as mock_from_dict:
        ...
        assert dbt_project.generated_at == "2021-08-01T00:00:00.000Z"


def test_get_model_nodes(dbt_project: DbtProject, mock_manifest_data: dict, tmp_path: Path) -> None:
    ...
    with patch("dbt_governance.dbt_project.Manifest.from_dict") as mock_from_dict:
        ...
        model_nodes = dbt_project.get_model_nodes()
        expected_nodes = {
