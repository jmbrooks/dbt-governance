...
def test_manifest_file_not_found(dbt_client: DbtClient) -> None:
    """Test behavior when the manifest file is not found."""
    with pytest.raises(FileNotFoundError, match="Manifest file not found"):
        dbt_client.load_manifest()


def test_get_dbt_version(dbt_client: DbtClient, mock_manifest_data: dict, tmp_path: Path) -> None:
    ...
    with patch("dbt_governance.dbt_client.Manifest.from_dict") as mock_from_dict:
        ...
        assert dbt_client.get_dbt_version() == "1.8.0"


def test_get_model_nodes(dbt_client: DbtClient, mock_manifest_data: dict, tmp_path: Path) -> None:
